...
export default function AddAddressDialog({ addAddressDialog, fetchShippingAddresses }) {
  ...
  const regionSelect = useSelectOptions(regions.map(region => region.name), 'Region')
  const provinceSelect = useSelectOptions(provinces.map(province => province.name), 'Municipality')
  const citySelect = useSelectOptions(cities.map(city => city.name), 'City')
  const barangaySelect = useSelectOptions(barangays.map(barangay => barangay.name), 'Barangay')
  ...
  useEffect(() => {
    let selectedRegion = regions.find(region => region.name == regionSelect.value)
    if (!selectedRegion) {
      return
  ...
  useEffect(() => {
    let selectedProvince = provinces.find(province => province.name == provinceSelect.value)
    if (!selectedProvince) {
      return
      ...
      <dialog ref={addAddressDialog.ref}>
        <div className='address-dialog box-shadow'>
          <form className='address-form'>
            ...
            <div className='barangay input-wrapper'>
              {barangaySelect.input}
            </div>
            ...
            <div className='default-address-checkbox flex-row justify-center'>
              <label >
                <input type="checkbox" onChange={e => setIsMainAddress(e.target.checked)} checked={isMainAddress} />
                &nbsp;&nbsp;Set as default
              </label>
            </div>
            <input className='submit-button' type="submit" onClick={handleAddressForm} value='Add' />
          </form>
        </div>
      </dialog>
    </>
