...
export default function PreviewSlider({ item }) {
  ...
  function slide(direction) {
    ...
    if (translation > -1 && translation < image_links.length - 2) {
      ...
      document.querySelector(".sub-slider").style.translate =
        `${sliderPos.current * -100}px`;
    }
  }
    ...
    <div className="slider-container flex-column align-center">
      ...
      <div className="sub-container flex-row justify-between align-center">
        ...
        <div className="sub-preview flex-row">
          <div className="sub-slider">{mapSubPreview(item)}</div>
        </div>
        <img
          id="rightNav"
          src="https://uxwing.com/wp-content/themes/uxwing/download/arrow-direction/angle-right-icon.png"
          alt="arrow-right"
          onClick={() => slide(1)}
        />
      </div>
